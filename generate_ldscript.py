...
def get_rnmis(harts):
    ...
    Cores may share same interrupt and exception handlers, so only
    unique handler sections (not same address) are generated.
    """
    rnmis = []
    rnmi_irqs = set()
    rnmi_excps = set()
    ...
    for hart in harts:
        rnmi_irq_addr = hart.get_fields("sifive,rnmi-irq-addr")
        rnmi_excp_addr = hart.get_fields("sifive,rnmi-exception-addr")

        if rnmi_irq_addr is not None and rnmi_excp_addr is not None:
            _rnmi_irq_addr = rnmi_irq_addr[1] << 32 | rnmi_irq_addr[0]
            _rnmi_excp_addr = rnmi_excp_addr[1] << 32 | rnmi_excp_addr[0]
